			...
			<div class="card">
				...
				function fill_table_reserve()
				{
					...
					if ($stmt = $conn->prepare("SELECT 	r_id,
						...
						echo "<table class='table table-bordered border-dark table-hover table-striped table-sm'>";
						...
						while ($stmt->fetch()) {
							...
							echo "<td>$r_adres</td>";
							echo "<td><a class='btn btn-outline-danger' href='all_reserve.php?delete_by_r_id=$r_id&action=delete' onclick='return confirm(\"آیا اطلاعات حذف شود؟\");'>حذف</a></td>";
							echo "<td><a class='btn btn-outline-primary' href='all_reserve.php?update_by_r_id=$r_id&action=display_form_update'>ویرایش</a></td>";
							echo "</tr>";
						}

						if ($stmt->num_rows == 0) {
							echo "<tr><td colspan='13' style='text-align:center; color:red;'><div  class='alert alert-danger center' role='alert'>موجود نیست ...!</td></div></tr></table></div></div></div>";
						}
						echo "</table>";
					} else {
				...
				?>
					<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
						...
						<div class="form" id="search">
							...
							<div style="float:right"><label>نام خانوادگی :</label><input type="text" class="form-control" name="search_r_fname" size="25" maxlength="25" value="<?php echo $search_r_fname ?>" /></div><br>
							<br><br>
						...
						<div class="card">
							<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
								...
								<div class="form" id="login">
									...
									<div style="clear:both"><label>آدرس :</label><textarea type="text" class="form-control" name="r_adres" rows="2" cols="100" required><?php echo $r_adres ?></textarea></div><br>
